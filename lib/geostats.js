...
var geostats = function(a) {
	...
	this.getClassStdDeviation = function(nbClass, matchBounds) {
	    ...
	    if(nbClass % 2 == 1) {
	    	...
	    	a[infBound] = mean - (stdDev / 2);
	    	a[supBound] = mean + (stdDev / 2);
	    	
	    	// Values < to infBound, except first one
	    	for (i = infBound - 1; i > 0; i--) {
	    		var val = a[i+1] - stdDev;
		        a[i] = val;
		    }
	    ...
	    a[0] = (typeof matchBounds === "undefined") ? a[1]- stdDev : tmpMin;
    	
    	// we finally set the last value
	    // do we excatly match max value or not ? 
    	a[nbClass] = (typeof matchBounds === "undefined") ? a[nbClass-1] + stdDev : tmpMax;
