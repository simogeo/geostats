(function(k){"object"===typeof exports?module.exports=k():"function"===typeof define&&define.amd?define(k):geostats=k()})(function(){var k=function(g){return"number"===typeof g&&parseFloat(g)==parseInt(g,10)&&!isNaN(g)};Array.prototype.indexOf||(Array.prototype.indexOf=function(g,a){if(void 0===this||null===this)throw new TypeError('"this" is null or not defined');var b=this.length>>>0;a=+a||0;Infinity===Math.abs(a)&&(a=0);0>a&&(a+=b,0>a&&(a=0));for(;a<b;a++)if(this[a]===g)return a;return-1});return function(g){this.objectId=
...
alert;var b="Given bounds may not be correct! please check your input.\nMin value : "+this.min()+" / Max value : "+this.max();a(b)}else return this.setBounds(a),this.setRanges(),this.method="manual classification ("+(a.length-1)+" classes)",this.bounds};this.getClassEqInterval=function(a){if(!this._nodata()){var b=this.max(),c=[],d=this.min(),e=(b-this.min())/a;for(i=0;i<=a;i++)c[i]=d,d+=e;c[a]=b;this.setBounds(c);this.setRanges();this.method="eq. intervals ("+a+" classes)";return this.bounds}};this.getClassQuantile=
function(a){if(!this._nodata()){var b=[],c=this.sorted(),d=Math.round(this.pop()/a),e=d,f=0;b[0]=c[0];for(f=1;f<a;f++)b[f]=c[e],e+=d;b.push(c[c.length-1]);this.setBounds(b);this.setRanges();this.method="quantile ("+a+" classes)";return this.bounds}};this.getClassStdDeviation=function(a){if(!this._nodata()){this.max();this.min();var b=[];if(1==a%2){var c=Math.floor(a/2),d=c+1;b[c]=this.mean()-this.stddev()/2;b[d]=this.mean()+this.stddev()/2;i=c-1}else d=a/2,b[d]=this.mean(),i=d-1;for(;0<i;i--)c=b[i+
1]-this.stddev(),b[i]=c;for(i=d+1;i<a;i++)c=b[i-1]+this.stddev(),b[i]=c;b[0]=this.min();b[a]=this.max();this.setBounds(b);this.setRanges();this.method="std deviation ("+a+" classes)";return this.bounds}};this.getClassGeometricProgression=function(a){if(!this._nodata())if(this._hasNegativeValue()||this._hasZeroValue())alert("geometric progression can't be applied with a serie containing negative or zero values.");else{var b=[],c=this.min(),d=this.max(),d=Math.log(d)/Math.LN10,c=Math.log(c)/Math.LN10,
d=(d-c)/a;for(i=0;i<a;i++)b[i]=0==i?c:b[i-1]+d;b=b.map(function(a){return Math.pow(10,a)});b.push(this.max());this.setBounds(b);this.setRanges();this.method="geometric progression ("+a+" classes)";return this.bounds}};this.getClassArithmeticProgression=function(a){if(!this._nodata()){var b=0;for(i=1;i<=a;i++)b+=i;var c=[],d=this.min(),b=(this.max()-d)/b;for(i=0;i<=a;i++)c[i]=0==i?d:c[i-1]+i*b;this.setBounds(c);this.setRanges();this.method="arithmetic progression ("+a+" classes)";return this.bounds}};
this.getClassJenks=function(a){if(!this._nodata()){dataList=this.sorted();for(var b=[],c=0,d=dataList.length+1;c<d;c++){for(var e=[],f=0,g=a+1;f<g;f++)e.push(0);b.push(e)}c=[];d=0;for(e=dataList.length+1;d<e;d++){for(var f=[],g=0,k=a+1;g<k;g++)f.push(0);c.push(f)}d=1;for(e=a+1;d<e;d++){b[0][d]=1;c[0][d]=0;for(var h=1,f=dataList.length+1;h<f;h++)c[h][d]=Infinity;h=0}d=2;for(e=dataList.length+1;d<e;d++){for(var k=g=f=0,m=1,q=d+1;m<q;m++){var n=d-m+1,h=parseFloat(dataList[n-1]),g=g+h*h,f=f+h,k=k+1,h=
g-f*f/k,p=n-1;if(0!=p)for(var l=2,r=a+1;l<r;l++)c[d][l]>=h+c[p][l-1]&&(b[d][l]=n,c[d][l]=h+c[p][l-1])}b[d][1]=1;c[d][1]=h}h=dataList.length;c=[];d=0;for(e=a+1;d<e;d++)c.push(0);c[a]=parseFloat(dataList[dataList.length-1]);c[0]=parseFloat(dataList[0]);for(d=a;2<=d;)e=parseInt(b[h][d]-2),c[d-1]=dataList[e],h=parseInt(b[h][d]-1),d-=1;c[0]==c[1]&&(c[0]=0);this.setBounds(c);this.setRanges();this.method="Jenks ("+a+" classes)";return this.bounds}};this.getClassUniqueValues=function(){if(!this._nodata()){this.is_uniqueValues=
...
this.getInnerRanges=function(){if(null!=this.inner_ranges)return this.inner_ranges;var a=[],b=this.sorted(),c=1;for(i=0;i<b.length;i++){if(0==i)var d=b[i];parseFloat(b[i])>parseFloat(this.bounds[c])&&(a[c-1]=""+d+this.separator+b[i-1],d=b[i],c++);if(c==this.bounds.length-1)return a[c-1]=""+d+this.separator+b[b.length-1],this.inner_ranges=a}};this.getSortedlist=function(){return this.sorted().join(", ")};this.getHtmlLegend=function(a,b,c,d,e){var f="";this.doCount();ccolors=null!=a?a:this.colors;lg=
null!=b?b:"Legend";getcounter=null!=c?!0:!1;fn=null!=d?d:function(a){return a};null==e&&(e="default");if("discontinuous"==e&&(this.getInnerRanges(),-1!==this.counter.indexOf(0))){alert("Geostats cannot apply 'discontinuous' mode to the getHtmlLegend() method because some classes are not populated.\nPlease switch to 'default' or 'distinct' modes. Exit!");return}if(ccolors.length<this.ranges.length)alert("The number of colors should fit the number of ranges. Exit!");else{a='<div class="geostats-legend"><div class="geostats-legend-title">'+
lg+"</div>";if(!1==this.is_uniqueValues)for(i=0;i<this.ranges.length;i++)!0===getcounter&&(f=' <span class="geostats-legend-counter">('+this.counter[i]+")</span>"),c=this.ranges[i].split(this.separator),b=parseFloat(c[0]).toFixed(this.precision),c=parseFloat(c[1]).toFixed(this.precision),"distinct"==e&&0!=i&&(k(b)?b=parseInt(b)+1:(b=parseFloat(b)+1/Math.pow(10,this.precision),b=parseFloat(b).toFixed(this.precision))),"discontinuous"==e&&(c=this.inner_ranges[i].split(this.separator),b=parseFloat(c[0]).toFixed(this.precision),
c=parseFloat(c[1]).toFixed(this.precision)),b=fn(b)+this.legendSeparator+fn(c),a+='<div><div class="geostats-legend-block" style="background-color:'+ccolors[i]+'"></div> '+b+f+"</div>";else for(i=0;i<this.bounds.length;i++)!0===getcounter&&(f=' <span class="geostats-legend-counter">('+this.counter[i]+")</span>"),b=fn(this.bounds[i]),a+='<div><div class="geostats-legend-block" style="background-color:'+ccolors[i]+'"></div> '+b+f+"</div>";return a+"</div>"}};this.objectID=(new Date).getUTCMilliseconds();
this.log("Creating new geostats object");"undefined"!==typeof g&&0<g.length?(this.serie=g,this.setPrecision(),this.log("Setting serie ("+g.length+") : "+g.join())):this.serie=[];this.getJenks=this.getClassJenks;this.getGeometricProgression=this.getClassGeometricProgression;this.getEqInterval=this.getClassEqInterval;this.getQuantile=this.getClassQuantile;this.getStdDeviation=this.getClassStdDeviation;this.getUniqueValues=this.getClassUniqueValues;this.getArithmeticProgression=this.getClassArithmeticProgression}});
